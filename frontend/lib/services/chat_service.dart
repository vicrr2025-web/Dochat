...
class ChatService {
  ...
  Future<PageResponse<SessionInfo>> getSessions({int page = 0, int size = 20}) async {
    final response = await _api.client.get('/v1/sessions', queryParameters: {
      'page': page,
      'size': size,
    });
    ...
    return PageResponse(content: [], page: 0, size: size, totalElements: 0, totalPages: 0, hasMore: false);
  }
  ...
  Future<void> muteSession(String sessionId, bool muted, {int? duration}) async {
    ...
    await _api.client.put(
      '/v1/sessions/$sessionId/mute',
      data: data,
    );
  }

  Future<void> deleteSession(String sessionId) async {
    await _api.client.delete('/v1/sessions/$sessionId');
  }

  Future<PageResponse<MessageInfo>> getMessages(
    String sessionId, {
  ...
  Future<void> revokeMessage(String messageId) async {
    await _api.client.put('/v1/messages/$messageId/revoke');
  }
